...
import dev.stay.util.render.TextUtil;
...
public class HUD extends Module {
    ...
    public void onRender2D(Render2DEvent event) {
        ...
        if (this.waterMark.getValue().booleanValue()) {
            String string = this.command.getPlannedValue() + ""; //version
    ...
    public void renderLag() {
        ...
        if (Stay.serverManager.isServerNotResponding()) {
            String text = ChatFormatting.RED + "Server not responding " + MathUtil.round((float) Stay.serverManager.serverRespondingTime() / 1000.0F, 1) + "s.";
