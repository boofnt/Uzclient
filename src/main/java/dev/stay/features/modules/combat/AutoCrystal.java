package dev.stay.features.modules.combat;
...
import dev.stay.event.events.UpdateWalkingPlayerEvent;
import dev.stay.features.command.Command;
import dev.stay.features.gui.StayGui;
import dev.stay.features.modules.Module;
import dev.stay.features.modules.misc.NoSoundLag;
import dev.stay.features.setting.Bind;
import dev.stay.features.setting.Setting;
import dev.stay.util.word.Timer;
import dev.stay.util.entity.EntityUtil;
import dev.stay.util.misc.MathUtil;
import dev.stay.util.word.BlockUtil;
import dev.stay.util.word.DamageUtil;
...
public class AutoCrystal
    ...
    public Setting<RenderMode> renderMode = this.register(new Setting("Mode", RenderMode.GLIDE, v -> this.setting.getValue() == Settings.RENDER && this.render.getValue()));;
    ...
    private final Setting<Float> moveSpeed = this.register(new Setting("Speed", 2000.0f, 1.0f, 2000.0f, v -> this.setting.getValue() == Settings.RENDER && this.renderMode.getValue() == RenderMode.GLIDE && this.render.getValue()));;
    private final Setting<Float> accel = this.register(new Setting("Deceleration", 0.1f, 0.0f, 1.0f, v -> this.setting.getValue() == Settings.RENDER && this.renderMode.getValue() == RenderMode.GLIDE && this.render.getValue()));;
    ...
    private final Setting<Integer> bRed = this.register(new Setting("BoxRed", 150, 0, 255, v -> this.setting.getValue() == Settings.RENDER && this.render.getValue() && this.box.getValue()));;
    private final Setting<Integer> bGreen = this.register(new Setting("BoxGreen", 0, 0, 255, v -> this.setting.getValue() == Settings.RENDER && this.render.getValue() && this.box.getValue()));;
    private final Setting<Integer> bBlue = this.register(new Setting("BoxBlue", 150, 0, 255, v -> this.setting.getValue() == Settings.RENDER && this.render.getValue() && this.box.getValue()));;
    private final Setting<Integer> bAlpha = this.register(new Setting("BoxAlpha", 40, 0, 255, v -> this.setting.getValue() == Settings.RENDER && this.render.getValue() && this.box.getValue()));;
    private final Setting<Boolean> boxrainbow = this.register(new Setting("Box Rainbow", false, v -> this.setting.getValue() == Settings.RENDER && this.render.getValue() && this.box.getValue()));;
    ...
    private final Setting<Integer> oRed = this.register(new Setting("OutlineRed", 255, 0, 255, v -> this.setting.getValue() == Settings.RENDER && this.render.getValue() && this.outline.getValue()));;
    private final Setting<Integer> oGreen = this.register(new Setting("OutlineGreen", 50, 0, 255, v -> this.setting.getValue() == Settings.RENDER && this.render.getValue() && this.outline.getValue()));;
    private final Setting<Integer> oBlue = this.register(new Setting("OutlineBlue", 255, 0, 255, v -> this.setting.getValue() == Settings.RENDER && this.render.getValue() && this.outline.getValue()));
    private final Setting<Integer> oAlpha = this.register(new Setting("OutlineAlpha", 255, 0, 255, v -> this.setting.getValue() == Settings.RENDER && this.render.getValue() && this.outline.getValue()));;
    private final Setting<Float> lineWidth = this.register(new Setting("LineWidth", 1.5f, 0.1f, 5.0f, v -> this.setting.getValue() == Settings.RENDER && this.render.getValue() && this.outline.getValue()));;
    private final Setting<Boolean> outlinerainbow = this.register(new Setting("Outline Rainbow", false, v -> this.setting.getValue() == Settings.RENDER && this.render.getValue() && this.outline.getValue()));;
    ...
    public void onPacketSend(PacketEvent.Send event) {
        ...
        if (event.getStage() == 0 && event.getPacket() instanceof CPacketUseEntity && (packet = event.getPacket()).getAction() == CPacketUseEntity.Action.ATTACK && packet.getEntityFromWorld(AutoCrystal.mc.world) instanceof EntityEnderCrystal) {
            ...
            if (this.antiBlock.getValue().booleanValue() && EntityUtil.isCrystalAtFeet(crystal, this.range.getValue().floatValue()) && pos != null) {
                ...
                BlockUtil.placeCrystalOnBlockCry(this.placePos, this.offHand ? EnumHand.OFF_HAND : EnumHand.MAIN_HAND, this.placeSwing.getValue(), this.exactHand.getValue());
            }
        }
    }
            ...
            if (packet2.getType() == 51 && AutoCrystal.mc.player.getDistanceSq(pos = new BlockPos(packet2.getX(), packet2.getY(), packet2.getZ())) + (double) this.predictOffset.getValue().floatValue() <= MathUtil.square(this.breakRange.getValue().floatValue()) && (this.instantTimer.getValue() == PredictTimer.NONE || this.instantTimer.getValue() == PredictTimer.BREAK && this.breakTimer.passedMs(this.breakDelay.getValue().intValue()) || this.instantTimer.getValue() == PredictTimer.PREDICT && this.predictTimer.passedMs(this.predictDelay.getValue().intValue()))) {
                ...
                if (this.predictFriendDmg.getValue().booleanValue() && (this.antiFriendPop.getValue() == AntiFriendPop.BREAK || this.antiFriendPop.getValue() == AntiFriendPop.ALL) && this.isRightThread()) {
                    for (EntityPlayer friend : AutoCrystal.mc.world.playerEntities) {
                        if (friend == null || AutoCrystal.mc.player.equals(friend) || friend.getDistanceSq(pos) > MathUtil.square(this.range.getValue().floatValue() + this.placeRange.getValue().floatValue()) || !Stay.friendManager.isFriend(friend) || !((double) DamageUtil.calculateDamage(pos, friend) > (double) EntityUtil.getHealth(friend) + 0.5))
                            continue;
                        return;
                    }
                }
    ...
    public void onKeyInput(InputEvent.KeyInputEvent event) {
        if (Keyboard.getEventKeyState() && !(AutoCrystal.mc.currentScreen instanceof StayGui) && this.switchBind.getValue().getKey() == Keyboard.getEventKey()) {
        ...
        for (Entity entity : AutoCrystal.mc.world.loadedEntityList) {
            ...
            if ((double) selfDamage + 0.5 < (double) EntityUtil.getHealth(AutoCrystal.mc.player) && selfDamage <= this.maxSelfBreak.getValue().floatValue()) {
                ...
                for (EntityPlayer player : AutoCrystal.mc.world.playerEntities) {
                    ...
                    if (this.antiFriendPop.getValue() != AntiFriendPop.BREAK && this.antiFriendPop.getValue() != AntiFriendPop.ALL || !Stay.friendManager.isFriend(player.getName()) || !((double) (damage = DamageUtil.calculateDamage(entity, player)) > (double) EntityUtil.getHealth(player) + 0.5))
                        continue;
        ...
        for (BlockPos pos : BlockUtil.possiblePlacePositions(this.placeRange.getValue().floatValue(), this.antiSurround.getValue(), this.oneDot15.getValue())) {
            ...
            if (targettedPlayer != null) {
                ...
                if (this.calcEvenIfNoDamage.getValue().booleanValue() && (this.antiFriendPop.getValue() == AntiFriendPop.ALL || this.antiFriendPop.getValue() == AntiFriendPop.PLACE)) {
                    ...
                    for (EntityPlayer friend : AutoCrystal.mc.world.playerEntities) {
                        float friendDamage;
                        if (friend == null || AutoCrystal.mc.player.equals(friend) || friend.getDistanceSq(pos) > MathUtil.square(this.range.getValue().floatValue() + this.placeRange.getValue().floatValue()) || !Stay.friendManager.isFriend(friend) || !((double) (friendDamage = DamageUtil.calculateDamage(pos, friend)) > (double) EntityUtil.getHealth(friend) + 0.5))
                            continue;
                        friendPop = true;
                        break;
                    }
    ...
    public static class PlaceInfo {
        ...
        public void runPlace() {
            BlockUtil.placeCrystalOnBlockCry(this.pos, this.offhand ? EnumHand.OFF_HAND : EnumHand.MAIN_HAND, this.placeSwing, this.exactHand);
        }
    }
